...
║     7. vector sampling حقيقي بـ pyarrow row groups                 ║
╚══════════════════════════════════════════════════════════════════════╝
"""
...
CONFIG = {
    ...
    "INPUT":  "gs://griidai-data/sample/maskedcog_v2.tif",
    "FORMAT": "png",   # geotiff | png | jpeg | jpeg2000 | geojson | shapefile | kml
...
def is_vector_input(gcs_input: str) -> bool:
    ...
    return gcs_input.lower().endswith(".parquet")
...
def build_prep_script(gcs_input: str, fmt: str, task_script: str,
    ...
    """
    local_input = "/tmp/input.parquet" if is_vector_input(gcs_input) else "/tmp/input.tif"
...
if __name__ == "__main__":
    ...
    print(f"🗂️  Input type : {'vector (parquet)' if is_vector_input(gcs_input) else 'raster (tif)'}")
    ...
    input_type    = sample_result.get("type", "unknown")

    print(f"   Input type            : {input_type}")
    print(f"   Peak RAM (sample)     : {peak_ram_gb:.3f} GB")
    print(f"   Estimated full RAM    : {needed_ram_gb:.1f} GB")

    if input_type == "vector":
        print(f"   Total rows            : {sample_result.get('total_rows', '?')}")
        print(f"   Row groups sampled    : {sample_result.get('groups_read', '?')} / {sample_result.get('total_groups', '?')}")
    elif input_type == "raster":
        tile = sample_result.get("tile", {})
        print(f"   Tile sampled          : {tile.get('x_size','?')}x{tile.get('y_size','?')} px")
